...
export default class Request extends Component {
    constructor(props){
      ...
      splitAmount = () => {
        ...
        const count = keep_senders.length;
        const average = Math.floor(sum / count);
        for(let i=0; i<count; i++) {
          const value = (i!=0) ? average : sum - (average * (count - 1));
          keep_senders[i].amount = value;
                  ...
                  </div>
                  {
                    this.state.keep_senders.map((r,i)=>(
                      <div key={r.id} className="input-group mb-1">
                        ...
                        <input id={`amount_${i}`} className="form-control" type="number" placeholder="請求金額" min="1" step="1" onChange={e=>this.updateSenderAmount(e.target.value,i)} required/>
