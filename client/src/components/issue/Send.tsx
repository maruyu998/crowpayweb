  ...
  function calcSplit(){
    ...
    if(isEnterSameAmount){
      ...
      for(const [userId, dealAmount] of Object.entries(dict)){
        const inp = document.getElementById(`sendDealAmount_${userId}`) as HTMLInputElement|null;
  ...
  async function submitHandler(){
    ...
    for(const friend of selectingFriendList){
      await issueTransaction({ 
        receiverUserId: friend.friendUserId,
  ...
  return (
    <div className="px-6">
      <div className="max-w-[48rem] mx-auto pb-12">
        ...
        <h2 className="text-lg">送金する相手を選択</h2>
        ...
        />
        {
          selectingFriendList.length > 0 && <>
                  ...
                  {selectingFriendList.map(friend=>(
                    <TableRow key={friend.friendUserId}>
                      ...
                      <TableCell className="p-2">
                        <NumberInput
                          id={`sendDealAmount_${friend.friendUserId}`}
            ...
            <p className="font-semibold text-center">
              合計送金金額: ¥ {Intl.NumberFormat('en-US').format(sum(selectingFriendList.map(f=>f.dealAmount??0)))}
            </p>
            <Divider className="my-4"/>
            <div className="flex justify-center">
              <Button variant="primary" onClick={submitHandler}>送金する</Button>
            </div>
          </>
        }
      </div>
